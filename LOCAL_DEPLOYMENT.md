...
账号域默认使用 `@dx-uestc`，通常无需修改。如果 `UESTC_USERNAME` 已经是包含 `@` 的完整用户名，程序不会重复追加账号域；只有校方明确要求其他域时，才在 `.env` 中设置 `UESTC_ACCOUNT_DOMAIN`。

运行中修改 `.env` 后，请在托盘菜单点击“立即检查并修复”，让守护线程重建会话并读取新配置。
...
- 临时网络或门户错误会在周期检查中退避重试；手动检查会重置退避并立即唤醒守护，门户要求等待的限流/临时锁定会在到期后自动恢复。
- 明确的账号或密码配置错误会暂停守护，避免持续尝试造成进一步锁定。
